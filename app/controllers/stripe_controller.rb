class StripeController < ApplicationController
  ...
  def handle_issuing_authorization_request(event)
    ...
    if should_approve
      ...
      StripeAuthorizationMailer.with(auth_obj: auth).notify_admin_of_approve.deliver_later
    else
      ...
      StripeAuthorizationMailer.with(auth_obj: auth).notify_admin_of_decline.deliver_later
    end
  end
