...
$(document).on("turbolinks:load", function() {
  ...
  window.updateChangelogTooltip = function() {
    const target = $("[data-behavior~=toggle_changelog]");
    if ($("#HW_frame_cont.HW_visible").length > 0) {
      return target.attr("aria-label", "");
    } else {
      return target.attr("aria-label", "Show changes");
    }
  };

  $(document).on(
    "mouseover",
    "[data-behavior~=toggle_changelog]",
    updateChangelogTooltip
  );
  ...
  // if you add the money behavior to an input, it'll add commas, only allow two numbers for cents, and only permit numbers to be entered
  $("input[data-behavior~=money]").on('input', function(e) {
    let value = $(this).val()
  ...
  BK.toggleMenu = function(m) {
    $(m)
      .find("[data-behavior~=menu_content]")
      .slideToggle(100);
