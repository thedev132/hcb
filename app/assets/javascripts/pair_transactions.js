...
document.addEventListener('DOMContentLoaded', () => {
    // Invoice payouts
    const invoicePayoutSelect = document.querySelector('.invoicepayout-select-target select');
    if (invoicePayoutSelect) {
        invoicePayoutSelect.addEventListener('change', evt => {
            const option = invoicePayoutSelect.querySelector(`option[value="${evt.target.value}"]`);
    ...
    if (lcrSelect) {
        lcrSelect.addEventListener('change', evt => {
            const option = lcrSelect.querySelector(`option[value="${evt.target.value}"]`);
            const optionText = option.textContent;
