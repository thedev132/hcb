...
;(function (factory) {
  ...
  $.modal.prototype = {
    ...
    block: function () {
      ...
      this.$blocker = $('.' + this.options.blockerClass)

      if (this.$blocker.length == 0) {
        this.$blocker = $(
          '<div class="' + this.options.blockerClass + '"></div>  '
        ).appendTo(this.$body)
      } else {
        this.$blocker = $($('.' + this.options.blockerClass)[0])
      }
      selectCurrent()
      if (this.options.doFade) {
        this.$blocker
          .addClass('blocker current')
          .css('display', '')
          .css('opacity', 0)
    ...
    unblock: function (now) {
      if (!now && this.options.doFade)
        ...
        this.$blocker.removeClass('blocker current')
        selectCurrent()
