...
RSpec.describe CheckService::Create, type: :model do
  # TODO: replace with factories once there is a lob_address factory. Because lob_addresses have to be linked
  # to an event, we are forced to use the events fixture here too.
  fixtures "events", "lob_addresses"
  ...
  context 'when an event has ample balance' do
    it "creates a check with the correct amount" do
      ...
      service = described_class.new(
        event_id: event.id,
        lob_address_id: lob_addresses(:lob_address1).id,
        payment_for: "",
        memo: "",
