...
RSpec.describe HashedTransactionService::PrimaryHash do
  let(:date) { '2020-09-15' }
  ...
  let(:service) { HashedTransactionService::PrimaryHash.new(attrs) }

  it 'hashes the combination' do
    expect(service.run).to eql(7400998678639308911)
  end
  ...
  context 'when amount_cents changes' do
    ...
    it 'changes the hash' do
      expect(service.run).to eql(13343312625259921839)
    end
  end
  ...
  context 'when amount_cents is negative' do
    ...
    it 'hashes' do
      expect(service.run).to eql(7600043190022159380)
    end
  end
