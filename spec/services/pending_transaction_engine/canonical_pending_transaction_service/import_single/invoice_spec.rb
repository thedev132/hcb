...
describe PendingTransactionEngine::CanonicalPendingTransactionService::ImportSingle::Invoice do
  ...
  context "when passing a raw pending invoice transaction that is not yet processed" do
    it "processes into a CanonicalPendingTransaction" do
      ...
      expect do
        described_class.new(raw_pending_invoice_transaction:).run
      end.to change { CanonicalPendingTransaction.count }.by(1)
    end
  end

  context "when passing a raw pending invoice transaction that is already processed" do
  ...
  context "attributes" do
    let(:raw_pending_invoice_transaction) { create(:raw_pending_invoice_transaction, date_posted: Date.current, amount_cents: 42) }
    ...
    context "when processed" do
      it "copies the attributes and sets fronted to true" do
        ...
        expect(canonical_pending_transaction.date).to eq(raw_pending_invoice_transaction.date_posted)
